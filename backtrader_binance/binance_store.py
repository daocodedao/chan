...
# os.environ["HTTP_PROXY"] = Util.getProxyDic()["http"]
# os.environ["HTTPS_PROXY"] = Util.getProxyDic()["http"]
...
class BinanceStore(object):
    ...
    def __init__(
        self, api_key, api_secret, coin_target, testnet=False, retries=5, tld="com"
    ):  # coin_refer, coin_target
        
        # session = requests.Session()
        # session.proxies.update(Util.getProxyDic())
        ...
        self.binance_socket = ThreadedWebsocketManager(
            ...
            api_secret,
            testnet=testnet,
            # requests_params={"proxy": "socks5://127.0.0.1:10809"},
        )
        # self.binance_socket = ThreadedWebsocketManager(api_key,
        #                                                api_secret,
        #                                                testnet=testnet, )
    ...
    def cancel_open_orders(self, symbol):
        ...
        if len(orders) > 0:
            self.binance._request_api(
                "delete", "openOrders", signed=True, data={"symbol": symbol}
            )
    ...
    def create_order(self, symbol, side, type, size, price):
        ...
        if type != ORDER_TYPE_MARKET:
            params.update({"price": self.format_price(symbol, price)})
    ...
    def get_balance(
        ...
        self._cash = free
        # api_logger.info(f"------binance free cash ----- {free}")
    ...
    def getdata(
        self, **kwargs
    ):  # timeframe, compression, start_date=None, end_date=None, LiveBars=True
        ...
        if symbol not in self._datas:
            self._datas[f"{symbol}{tf}"] = BinanceData(
                store=self, **kwargs
            )  # timeframe=timeframe, compression=compression, start_date=start_date, end_date=None, LiveBars=LiveBars
